...
public class PageVO {
	...
	// Integer 웹에서 받은 페이지 정보(String)가 없으면 null인데 int는 null을 저장할 수 없다. 오류방지
	// client가 요청하는 페이지 번호를 String으로 저장하지만, int로 인식하는 객체형 정수타입이 Integer(int가 아님을 주의)
	private int totalCount;
	// 전체 게시글의 수
	
	// 페이지당 출력할 페이지 번호수(첫화면이 1~10페이지)
	private int endPage;	// 13페이지 있는 곳은 끝번호가 20까지
	private int startPage;  // 13페이지 있는 곳은 시작 번호가 11부터
	private boolean prev;
	private boolean next;
	
	// 검색을 했을때 해당되는 게시글에 따른 페이징 처리를 하기 위해서는 따로 변수가 필요하다
	// 또한 검색어와 검색한 페이지 둘다 파라미터로 넘겨야하므로 VO객체로 같이 넘겨줘야한다. 
	// 따라서 검색용 변수 2개 추가하여 사용
